...
public class AutoHandler {
  ...
  public void periodic() {
    ...
    String newPathName = PathPlannerAuto.currentPathName;
    ...
    try {
      ...
      double sampleTime =
          MathUtil.clamp(timeOnPath + LOOKAHEAD_SECONDS, 0.0, trajectory.getTotalTimeSeconds());
      ...
      if (recoveryState == RecoveryState.IDLE) {
        ...
        double recoverySampleTime =
            MathUtil.clamp(
                recoveryTargetTime + LOOKAHEAD_SECONDS, 0.0, trajectory.getTotalTimeSeconds());

        PathPlannerTrajectoryState targetState =
            (AutoBuilder.shouldFlip())
                ? trajectory.sample(recoverySampleTime).flip()
                : trajectory.sample(recoverySampleTime);
                            ...
                            .withSpeeds(
                                robotContainer
                                    ...
                                    .calculateRobotRelativeSpeeds(
                                        robotContainer.drivetrain.getState().Pose, targetState))));

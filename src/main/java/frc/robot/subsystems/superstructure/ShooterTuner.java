...
public class ShooterTuner {
  ...
  private AngularSubsystem otherFlywheel;
  private Hood hood;
  private AngularSubsystem turret;
  private final ShooterHandler shooterHandler;
  private final ShooterConfig config;

  @AutoLogOutput(key = "{name}/goal")
  @Setter
  @Getter
  private Goal goal = Goal.NONE;

  @AutoLogOutput(key = "{name}/lastShotResult")
  @Getter
  private ShotResult lastShotResult = ShotResult.HIT;

  @AutoLogOutput(key = "{name}/indexing")
  @Getter
  private boolean indexing = false;

  @AutoLogOutput(key = "{name}/flywheelSpeed")
  private AngularVelocity flywheelSpeed = RotationsPerSecond.of(RESET_FLYWHEEL_RPS);

  @AutoLogOutput(key = "{name}/requiredExitSpeed")
  private LinearVelocity requiredExitSpeed = MetersPerSecond.of(0);

  private static final double MIN_FLYWHEEL_RPS = 0.0, MAX_FLYWHEEL_RPS = 100.0;
  private static final double RESET_FLYWHEEL_RPS = 50;

  @AutoLogOutput(key = "{name}/flywheelMinValue")
  private double flywheelMinValue = MIN_FLYWHEEL_RPS;

  @AutoLogOutput(key = "{name}/flywheelMaxValue")
  private double flywheelMaxValue = MAX_FLYWHEEL_RPS;

  private final Stack<TunerState> flywheelHistory = new Stack<>();

  private final AngularVelocity OTHER_FLYWHEEL_SPEED = RotationsPerSecond.of(15);
  ...
  public ShooterTuner(
      ...
      AngularSubsystem otherFlywheel,
      Hood hood,
      AngularSubsystem turret,
      ShooterHandler shooterHandler) {
    this.name = name;
    this.flywheel = flywheel;
    this.otherFlywheel = otherFlywheel;
    this.hood = hood;
    this.turret = turret;
  ...
  public void periodic() {
    ...
    if (Controllers.UNDERSHOOT.rising()) {

      applyShotResult(ShotResult.UNDERSHOOT);
    }
    ...
    otherFlywheel.setVelocity(OTHER_FLYWHEEL_SPEED);
  }
  ...
  private LinearVelocity computeExitSpeed() {
    ...
    Logger.recordOutput(name + "/Distance", distance);
  ...
  private void reset() {
    System.out.println(name + " SHOT TABLE ENTRIES");
